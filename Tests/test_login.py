...
def test_page(page: Page):
    ...
    expect(page).to_have_text("Login to your account")
...
def test_invalid_user_login(page: Page, users: dict):
    login_page = LoginPage(page)
    login_page.login(email = users["username"], password = users["password"])

    expect(page).to_have_text("Your email or password is incorrect!")
